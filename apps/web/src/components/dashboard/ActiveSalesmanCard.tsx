...
export default function ActiveSalesmanCard() {
  const activeCount = 5;
    ...
    <div className="bg-white rounded-card shadow-card p-6 h-full">
      ...
      <div className="flex items-center gap-2">
        {[...Array(displayCount)].map((_, i) => (
          <div
            key={i}
            className="w-12 h-12 rounded-full bg-gradient-to-br from-primary-yellow to-primary-yellow-dark flex items-center justify-center text-white -ml-2 first:ml-0 border-2 border-white"
          >
            <User size={20} />
          </div>
        ))}
